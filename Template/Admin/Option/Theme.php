	...
	<div class="layui-row content-body place-holder" id="layer-photos">
		...
		<div class="layui-col-lg12">
			<div class="setting-msg">
				<p>1. 为了分摊服务器成本，主题下载和更新需要订阅用户才能使用，敬请谅解！订阅地址：<a href="https://opennav.soraharu.com/license/" rel="nofollow" target="_blank" title="购买订阅服务">https://opennav.soraharu.com/license/</a></p>
				<p>2. 部分主题来自其它开源项目，OpenNav 仅做适配，主题版权归原作者所有</p>
			</div>
		</div>
		<!-- 说明提示框 END -->
		<div class="layui-col-lg12">
...
<script>
	...
	function set_theme(name) {
		$.post("./index.php?c=api&method=set_theme", {
			key: "theme",
			value: name
		},
	...
	function download_theme(name, type) {
		// 请求订阅接口，并获取 key、value
		var index = layer.load(1);
		$.get("./index.php?c=api&method=check_subscribe",
		function(result, status) {
			if (result.code == 200) {
				var key = result.data.key;
				var value = result.data.value;
				// 继续下载主题
				$.get("./index.php?c=api&method=download_theme", {
					name: name,
					key: key,
					value: value,
					type: type
	...
	function update_theme(name, version) {
		// 获取远程主题最新版本号
		var index = layer.load(1);
		var infourl = "https://gitlab.soraharu.com/XiaoXi/OpenNav/-/raw/main/Public/themes/" + name + "/info.json";
		$.ajax({
			type: "HEAD",
			async: true,
			url: infourl,
			statusCode: {
				200 : function() {
					$.get("https://gitlab.soraharu.com/XiaoXi/OpenNav/-/raw/main/Public/themes/" + name + "/info.json",
					function(data, status) {
						let new_version = data.version;
						if (version >= new_version) {
							layer.closeAll('loading');
							layer.msg('已经是最新版本，无需更新！', {
								icon: 5
							});
						} else {
							download_theme(name, 'update');
						}
					});
				},
				403 : function() {
	...
	layer.photos({
		photos: '#layer-photos',
		anim: 5 // 0-6 的选择，指定弹出图片动画类型，默认随机
	});
</script>
